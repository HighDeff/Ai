...
def clean_response_text(text):
    """Clean response text by removing metadata, duplicates, and noise."""
    if not text:
        return ""
    import re
    
    # Remove patterns like [3] source=variation_direct score=0.840
    text = re.sub(r'\[[\d]+]\s*source=[^\[]*', ' ', text, flags=re.IGNORECASE)
    
    # Remove [2] source=cognitive_simplified score=0.880 Index:
    text = re.sub(r'\[2\]\s*source=[^\[]*Index:', ' ', text, flags=re.IGNORECASE)
    
    # Remove Index: patterns
    text = re.sub(r'\bIndex:\s*', ' ', text)
    
    # Remove [score=0.xxx | source=xxx | consistency=0.x]
    text = re.sub(r'\[score=[\d.]+\s*\|[^]]+\]', ' ', text)
    
    # Remove connector numbers: Furthermore, 880, Plus, 1, Moreover, 2
    text = re.sub(r',\s*\d{2,4}\s*,', ',', text)
    text = re.sub(r'\bFurthermore,\s*\d+\s*', ' ', text)
    text = re.sub(r'\bPlus,\s*\d+\s*', ', ', text)
    text = re.sub(r'\bMoreover,\s*\d+\s*', ', ', text)
    text = re.sub(r'\bAdditionally,\s*\d+\s*', ', ', text)
    
    # Remove garbled/incomplete connectors
    text = re.sub(r'\bFurthermore,\s*$', ' ', text)
    text = re.sub(r'\bMoreover,\s*$', ' ', text)
    text = re.sub(r'\bAdditionally,\s*$', ' ', text)
    text = re.sub(r'\bPlus,\s*$', ' ', text)
    
    # Remove question patterns (internal tracking)
    text = re.sub(r'\s+Q:\s*What is the [a-z]+\'[a-z]+ [a-z]+', ' ', text)
    text = re.sub(r'\bWhat is the \w+\'\w+s? \w+\?', ' ', text)
    
    # Remove garbled text
    text = re.sub(r'\s*You can catch specific exceptions[^\.]*', ' ', text)
    
    # Fix article grammar
    text = re.sub(r'\ba \s+([aeiouAEIOU]\w+)\b', r'an \1', text)
    ...
    unique = []
    for s in sentences:
        # Skip incomplete sentences (ending with "to" or "for" or "and")
        if s.endswith(' to') or s.endswith(' for') or s.endswith(' and'):
            continue
        key = re.sub(r'[^\w\s]', '', s.lower())[:50]
        if key not in seen:
            seen.add(key)
            unique.append(s)
    text = '. '.join(unique)
...
class ResponseGenerator:
    ...
    def generate_variations(self, entity, query, candidates, num_variations=3,
        ...
        # Clean candidates before processing
        cleaned = []
        for c in candidates:
            if isinstance(c, dict):
                c = dict(c)  # copy
                c['text'] = clean_response_text(c.get('text', ''))
                if c['text']:
                    cleaned.append(c)
            else:
                cleaned_text = clean_response_text(str(c))
                if cleaned_text:
                    cleaned.append({'text': cleaned_text, 'score': 0.5})
        candidates = cleaned
        ...
        print(f"[DEBUG] generate_variations: received {len(candidates)} candidates", flush=True)
        print(f"[DEBUG] Candidates: {[c.get('text', '')[:50] for c in candidates[:3]]}", flush=True)
        ...
        # Strategy 7: GoalSeekingComposer approach
        if self.goal_composer and entity:
            goal_resp = self.goal_composer.compose(entity, query, max_sentences=4)
            if goal_resp:
                variations.append({
                    "text": goal_resp,
                    "source": "goal_seeking",
        ...
        # Strategy 8: CognitiveGrid analysis
        if hasattr(self, 'cognitive_grid') and entity:
            cog = self.cognitive_grid.analyze(entity, query)
            if cog:
                variations.append({
                    "text": cog,
        ...
        # Strategy 9: EmotionalState tone
        if hasattr(self, 'emotional_state') and entity:
            emotion_resp = self.emotional_state.generate_response(entity, query)
            if emotion_resp:
                variations.append({
                    "text": emotion_resp,
        ...
        # Strategy 10: Entity comparison
        if entity and self.ke:
            comp = self._compare_entity(entity, query, candidates)
            if comp:
                variations.append({
                    "text": comp,
                    "source": "entity_comparison",
                    "score": 0.72,
                    "style": "comparison",
                })

        # Strategy 11: Temporal context
        if entity and self.ke:
            temporal = self._get_temporal_context(entity, query)
            if temporal:
                variations.append({
                    "text": temporal,
        ...
        # Strategy 12: Property-based response
        prop_resp = self._property_based_response(entity, query, candidates)
        if prop_resp:
            variations.append({
                "text": prop_resp,
        ...
        # Strategy 13: Attribute-based response
        attr_resp = self._attribute_based_response(entity, query, candidates)
        if attr_resp:
            variations.append({
                "text": attr_resp,
                "source": "attribute_based",
                "score": 0.69,
                "style": "attributes",
            })

        # Strategy 14: Similar entity reference
        similar = self._find_similar_entity(entity, query)
        if similar:
            variations.append({
                "text": similar,
        ...
        # Strategy 15: Category-based response
        cat_resp = self._category_based_response(entity, query)
        if cat_resp:
            variations.append({
                "text": cat_resp,
        ...
        # Strategy 16: Relationship-based response
        rel_resp = self._relationship_based_response(entity, query)
        if rel_resp:
            variations.append({
                "text": rel_resp,
        ...
        # Strategy 17: Example-based response
        ex_resp = self._example_based_response(entity, query, candidates)
        if ex_resp:
            variations.append({
                "text": ex_resp,
        ...
        # Strategy 18: Contrast-based response
        contrast_resp = self._contrast_based_response(entity, query, candidates)
        if contrast_resp:
            variations.append({
                "text": contrast_resp,
                "source": "contrast_based",
                "score": 0.64,
                "style": "contrast",
            })

        # Strategy 19: Historical context
        hist_resp = self._historical_context(entity, query)
        if hist_resp:
            variations.append({
                "text": hist_resp,
        ...
        # Strategy 20: Usage-based response
        usage_resp = self._usage_based_response(entity, query)
        if usage_resp:
            variations.append({
                "text": usage_resp,
                "source": "usage_based",
        ...
        # Strategy 21: Origin-based response
        origin_resp = self._origin_based_response(entity, query)
        if origin_resp:
            variations.append({
                "text": origin_resp,
                "source": "origin_based",
        ...
        # Strategy 22: Comparison with common knowledge
        common_resp = self._common_knowledge_response(entity, query)
        if common_resp:
            variations.append({
                "text": common_resp,
        ...
        # Strategy 23: Scientific classification
        sci_resp = self._scientific_classification(entity, query)
        if sci_resp:
            variations.append({
                "text": sci_resp,
        ...
        # Strategy 24: Fun fact response
        fun_resp = self._fun_fact_response(entity, query, candidates)
        if fun_resp:
            variations.append({
                "text": fun_resp,
                "source": "fun_fact",
        ...
        # Strategy 25: Warning/caution response
        warn_resp = self._warning_response(entity, query)
        if warn_resp:
            variations.append({
                "text": warn_resp,
                "source": "warning_response",
                "score": 0.57,
                "style": "warnings",
            })

        # Strategy 26: Benefit-based response
        benefit_resp = self._benefit_based_response(entity, query)
        if benefit_resp:
            variations.append({
                "text": benefit_resp,
                "source": "benefit_based",
                "score": 0.56,
                "style": "benefits",
            })

        # Strategy 27: Characteristic summary
        char_resp = self._characteristic_summary(entity, query, candidates)
        if char_resp:
            variations.append({
                "text": char_resp,
                "source": "characteristic_summary",
                "score": 0.55,
                "style": "characteristics",
            })

        # Strategy 28: Quick fact response
        quick_resp = self._quick_fact_response(entity, query, candidates)
        if quick_resp:
            variations.append({
                "text": quick_resp,
                "source": "quick_fact",
        ...
        # Strategy 29: Detailed breakdown
        detail_resp = self._detailed_breakdown(entity, query, candidates)
        if detail_resp:
            variations.append({
                "text": detail_resp,
                "source": "detailed_breakdown",
                "score": 0.53,
                "style": "detailed",
            })

        # Strategy 30: Simple summary
        simple_resp = self._simple_summary(entity, query, candidates)
        if simple_resp:
            variations.append({
                "text": simple_resp,
        ...
        # Strategy 31: Related concept
        rel_resp = self._related_concept(entity, query)
        if rel_resp:
            variations.append({
                "text": rel_resp,
        ...
        # Strategy 32: Interesting fact
        interest_resp = self._interesting_fact(entity, query, candidates)
        if interest_resp:
            variations.append({
                "text": interest_resp,
                "source": "interesting_fact",
                "score": 0.50,
                "style": "interesting",
            })

        # Strategy 33: Practical application
        prac_resp = self._practical_application(entity, query)
        if prac_resp:
            variations.append({
                "text": prac_resp,
                "source": "practical_application",
                "score": 0.49,
                "style": "practical",
            })

        # Strategy 34: Cultural significance
        cult_resp = self._cultural_significance(entity, query)
        if cult_resp:
            variations.append({
                "text": cult_resp,
                "source": "cultural_significance",
                "score": 0.48,
                "style": "cultural",
            })

        # Strategy 35: Technical detail
        tech_resp = self._technical_detail(entity, query, candidates)
        if tech_resp:
            variations.append({
                "text": tech_resp,
        ...
        # Strategy 36: Visual description
        vis_resp = self._visual_description(entity, query)
        if vis_resp:
            variations.append({
                "text": vis_resp,
        ...
        # Strategy 37: Comparative analysis
        comp_resp = self._comparative_analysis(entity, query)
        if comp_resp:
            variations.append({
                "text": comp_resp,
        ...
        # Strategy 38: Statistics-based response
        stat_resp = self._statistics_response(entity, query, candidates)
        if stat_resp:
            variations.append({
                "text": stat_resp,
                "source": "statistics_response",
                "score": 0.44,
                "style": "statistics",
            })

        # Strategy 39: Timeline response
        timeline_resp = self._timeline_response(entity, query)
        if timeline_resp:
            variations.append({
                "text": timeline_resp,
                "source": "timeline_response",
                "score": 0.43,
                "style": "timeline",
            })

        # Strategy 40: Pros and cons
        pros_resp = self._pros_cons_response(entity, query)
        if pros_resp:
            variations.append({
                "text": pros_resp,
                "source": "pros_cons",
        ...
        # Strategy 41: Myth vs reality
        myth_resp = self._myth_reality(entity, query)
        if myth_resp:
            variations.append({
                "text": myth_resp,
                "source": "myth_reality",
        ...
        # Strategy 42: Expert opinion
        expert_resp = self._expert_opinion(entity, query)
        if expert_resp:
            variations.append({
                "text": expert_resp,
        ...
        # Strategy 43: Common misconceptions
        mis_resp = self._common_misconceptions(entity, query)
        if mis_resp:
            variations.append({
                "text": mis_resp,
        ...
        # Strategy 44: Real-world example
        real_resp = self._real_world_example(entity, query)
        if real_resp:
            variations.append({
                "text": real_resp,
                "source": "real_world_example",
                "score": 0.38,
                "style": "real_world",
            })

        # Strategy 45: Key takeaway
        takeaway_resp = self._key_takeaway(entity, query, candidates)
        if takeaway_resp:
            variations.append({
                "text": takeaway_resp,
                "source": "key_takeaway",
        ...
        # Strategy 46: Summary with emphasis
        emph_resp = self._summary_with_emphasis(entity, query, candidates)
        if emph_resp:
            variations.append({
                "text": emph_resp,
                "source": "summary_emphasis",
                "score": 0.36,
                "style": "emphasis",
            })

        # Strategy 47: Narrative response
        narr_resp = self._narrative_response(entity, query, candidates)
        if narr_resp:
            variations.append({
                "text": narr_resp,
        ...
        # Strategy 48: Concise answer
        conc_resp = self._concise_answer(entity, query, candidates)
        if conc_resp:
            variations.append({
                "text": conc_resp,
                "source": "concise_answer",
                "score": 0.34,
                "style": "concise",
            })

        # Strategy 49: Elaborate response
        elab_resp = self._elaborate_response(entity, query, candidates)
        if elab_resp:
            variations.append({
                "text": elab_resp,
                "source": "elaborate_response",
                "score": 0.33,
                "style": "elaborate",
            })

        # Strategy 50: Question-based response
        quest_resp = self._question_based_response(entity, query, candidates)
        if quest_resp:
            variations.append({
                "text": quest_resp,
        ...
        # Strategy 51: Definition response
        def_resp = self._definition_response(entity, query, candidates)
        if def_resp:
            variations.append({
                "text": def_resp,
                "source": "definition_response",
                "score": 0.31,
                "style": "definition",
            })

        # Strategy 52: Cause and effect
        cause_resp = self._cause_effect(entity, query)
        if cause_resp:
            variations.append({
                "text": cause_resp,
                "source": "cause_effect",
        ...
        # Strategy 53: Step-by-step explanation
        step_resp = self._step_by_step(entity, query, candidates)
        if step_resp:
            variations.append({
                "text": step_resp,
                "source": "step_by_step",
        ...
        # Strategy 54: Analogy response
        analogy_resp = self._analogy_response(entity, query)
        if analogy_resp:
            variations.append({
                "text": analogy_resp,
                "source": "analogy_response",
                "score": 0.28,
                "style": "analogy",
            })

        # Strategy 55: Expert tip
        tip_resp = self._expert_tip(entity, query)
        if tip_resp:
            variations.append({
                "text": tip_resp,
                "source": "expert_tip",
        ...
        # Strategy 56: Common use case
        use_resp = self._common_use_case(entity, query)
        if use_resp:
            variations.append({
                "text": use_resp,
                "source": "common_use_case",
                "score": 0.26,
                "style": "use_case",
            })

        # Strategy 57: Related field
        field_resp = self._related_field(entity, query)
        if field_resp:
            variations.append({
                "text": field_resp,
        ...
        # Strategy 58: Etymology
        etym_resp = self._etymology_response(entity, query)
        if etym_resp:
            variations.append({
                "text": etym_resp,
                "source": "etymology_response",
                "score": 0.24,
                "style": "etymology",
            })

        # Strategy 59: Related phenomenon
        phen_resp = self._related_phenomenon(entity, query)
        if phen_resp:
            variations.append({
                "text": phen_resp,
                "source": "related_phenomenon",
                "score": 0.23,
                "style": "phenomenon",
            })

        # Strategy 60: Key principle
        principle_resp = self._key_principle(entity, query)
        if principle_resp:
            variations.append({
                "text": principle_resp,
                "source": "key_principle",
                "score": 0.22,
                "style": "principle",
            })

        # Strategy 61: Fundamental concept
        fund_resp = self._fundamental_concept(entity, query)
        if fund_resp:
            variations.append({
                "text": fund_resp,
                "source": "fundamental_concept",
                "score": 0.21,
                "style": "fundamental",
            })

        # Strategy 62: Practical tip
        prat_resp = self._practical_tip(entity, query)
        if prat_resp:
            variations.append({
                "text": prat_resp,
                "source": "practical_tip",
                "score": 0.20,
                "style": "practical_tip",
            })

        # Strategy 63: Common mistake
        mistake_resp = self._common_mistake(entity, query)
        if mistake_resp:
            variations.append({
                "text": mistake_resp,
        ...
        # Strategy 64: Best practice
        best_resp = self._best_practice(entity, query)
        if best_resp:
            variations.append({
                "text": best_resp,
        ...
        # Strategy 65: Key metric
        metric_resp = self._key_metric(entity, query, candidates)
        if metric_resp:
            variations.append({
                "text": metric_resp,
                "source": "key_metric",
        ...
        # Strategy 66: Industry standard
        ind_resp = self._industry_standard(entity, query)
        if ind_resp:
            variations.append({
                "text": ind_resp,
        ...
        # Strategy 67: Historical development
        histdev_resp = self._historical_development(entity, query)
        if histdev_resp:
            variations.append({
                "text": histdev_resp,
        ...
        # Strategy 68: Modern interpretation
        mod_resp = self._modern_interpretation(entity, query)
        if mod_resp:
            variations.append({
                "text": mod_resp,
                "source": "modern_interpretation",
                "score": 0.14,
                "style": "modern",
            })

        # Strategy 69: Scientific principle
        sciprincip_resp = self._scientific_principle(entity, query)
        if sciprincip_resp:
            variations.append({
                "text": sciprincip_resp,
                "source": "scientific_principle",
                "score": 0.13,
                "style": "scientific_principle",
            })

        # Strategy 70: Real-world application
        realapp_resp = self._realworld_application(entity, query)
        if realapp_resp:
            variations.append({
                "text": realapp_resp,
                "source": "realworld_application",
                "score": 0.12,
                "style": "realworld_app",
            })

        # Strategy 71: Conceptual explanation
        concept_resp = self._conceptual_explanation(entity, query)
        if concept_resp:
            variations.append({
                "text": concept_resp,
                "source": "conceptual_explanation",
                "score": 0.11,
                "style": "conceptual",
            })

        # Strategy 72: Practical implication
        impl_resp = self._practical_implication(entity, query)
        if impl_resp:
            variations.append({
                "text": impl_resp,
                "source": "practical_implication",
                "score": 0.10,
                "style": "implication",
            })

        # Strategy 73: Theoretical basis
        theory_resp = self._theoretical_basis(entity, query)
        if theory_resp:
            variations.append({
                "text": theory_resp,
                "source": "theoretical_basis",
                "score": 0.09,
                "style": "theoretical",
            })

        # Strategy 74: Empirical evidence
        empir_resp = self._empirical_evidence(entity, query)
        if empir_resp:
            variations.append({
                "text": empir_resp,
        ...
        # Strategy 75: Comparative perspective
        comppers_resp = self._comparative_perspective(entity, query)
        if comppers_resp:
            variations.append({
                "text": comppers_resp,
                "source": "comparative_perspective",
                "score": 0.07,
                "style": "perspective",
            })

        # Strategy 76: Historical precedent
        precedent_resp = self._historical_precedent(entity, query)
        if precedent_resp:
            variations.append({
                "text": precedent_resp,
        ...
        # Strategy 77: Current trends
        trends_resp = self._current_trends(entity, query)
        if trends_resp:
            variations.append({
                "text": trends_resp,
        ...
        # Strategy 78: Future outlook
        outlook_resp = self._future_outlook(entity, query)
        if outlook_resp:
            variations.append({
                "text": outlook_resp,
                "source": "future_outlook",
                "score": 0.04,
                "style": "outlook",
            })

        # Strategy 79: Key challenges
        challenges_resp = self._key_challenges(entity, query)
        if challenges_resp:
            variations.append({
                "text": challenges_resp,
                "source": "key_challenges",
                "score": 0.03,
                "style": "challenges",
            })

        # Strategy 80: Success factors
        success_resp = self._success_factors(entity, query)
        if success_resp:
            variations.append({
                "text": success_resp,
        ...
        # Strategy 81: Risk factors
        risk_resp = self._risk_factors(entity, query)
        if risk_resp:
            variations.append({
                "text": risk_resp,
                "source": "risk_factors",
        ...
        # Strategy 82: Environmental factors
        env_resp = self._environmental_factors(entity, query)
        if env_resp:
            variations.append({
                "text": env_resp,
                "source": "environmental_factors",
                "score": 0.50,
                "style": "environmental",
            })

        # Strategy 83: Social impact
        social_resp = self._social_impact(entity, query)
        if social_resp:
            variations.append({
                "text": social_resp,
        ...
        # Strategy 84: Economic factors
        econ_resp = self._economic_factors(entity, query)
        if econ_resp:
            variations.append({
                "text": econ_resp,
        ...
        # Strategy 85: Political factors
        pol_resp = self._political_factors(entity, query)
        if pol_resp:
            variations.append({
                "text": pol_resp,
        ...
        # Strategy 86: Cultural factors
        cultfac_resp = self._cultural_factors(entity, query)
        if cultfac_resp:
            variations.append({
                "text": cultfac_resp,
                "source": "cultural_factors",
                "score": 0.50,
                "style": "cultural_factors",
            })

        # Strategy 87: Technological factors
        techfac_resp = self._technological_factors(entity, query)
        if techfac_resp:
            variations.append({
                "text": techfac_resp,
                "source": "technological_factors",
                "score": 0.50,
                "style": "technological",
            })

        # Strategy 88: Legal considerations
        legal_resp = self._legal_considerations(entity, query)
        if legal_resp:
            variations.append({
                "text": legal_resp,
                "source": "legal_considerations",
                "score": 0.50,
                "style": "legal",
            })

        # Strategy 89: Ethical considerations
        ethical_resp = self._ethical_considerations(entity, query)
        if ethical_resp:
            variations.append({
                "text": ethical_resp,
        ...
        # Strategy 90: Global perspective
        global_resp = self._global_perspective(entity, query)
        if global_resp:
            variations.append({
                "text": global_resp,
                "source": "global_perspective",
                "score": 0.50,
                "style": "global",
            })

        # Strategy 91: Learning intent detection
        learn_resp = self._detect_learning_intent(entity, query)
        if learn_resp:
            variations.append({
                "text": learn_resp,
        ...
        # Strategy 92: Entity extraction
        entity_resp = self._extract_entity_info(entity, query, candidates)
        if entity_resp:
            variations.append({
                "text": entity_resp,
        ...
        # Strategy 93: Relative time context
        time_resp = self._get_relative_time_context(entity, query)
        if time_resp:
            variations.append({
                "text": time_resp,
        ...
        # Strategy 94: Health computation
        health_resp = self._compute_health_assessment(entity, query)
        if health_resp:
            variations.append({
                "text": health_resp,
        ...
        # Strategy 95: Goal creation
        goal_resp = self._create_goal_tracking(entity, query)
        if goal_resp:
            variations.append({
                "text": goal_resp,
        ...
        # Strategy 96: Progress update
        progress_resp = self._update_progress(entity, query)
        if progress_resp:
            variations.append({
                "text": progress_resp,
        ...
        # Strategy 97: Continuation addition
        cont_resp = self._add_continuation(entity, query, candidates)
        if cont_resp:
            variations.append({
                "text": cont_resp,
                "source": "continuation",
        ...
        # Strategy 98: Followup question
        followup_resp = self._add_followup(entity, query)
        if followup_resp:
            variations.append({
                "text": followup_resp,
                "source": "followup",
        ...
        # Strategy 99: Create plan
        plan_resp = self._create_plan(entity, query)
        if plan_resp:
            variations.append({
                "text": plan_resp,
        ...
        # Strategy 100: Format duration
        dur_resp = self._format_duration_response(entity, query)
        if dur_resp:
            variations.append({
                "text": dur_resp,
                "source": "duration_format",
                "score": 0.50,
                "style": "duration",
            })

        # Strategy 101: Format value
        val_resp = self._format_value_response(entity, query)
        if val_resp:
            variations.append({
                "text": val_resp,
                "source": "value_format",
        ...
        # Strategy 102: Add detail
        detail_resp = self._add_detail(entity, query, candidates)
        if detail_resp:
            variations.append({
                "text": detail_resp,
                "source": "add_detail",
        ...
        # Strategy 103: Add examples
        examp_resp = self._add_examples(entity, query, candidates)
        if examp_resp:
            variations.append({
                "text": examp_resp,
                "source": "add_examples",
        ...
        # Strategy 104: Confidence computation
        conf_resp = self._compute_confidence(entity, query)
        if conf_resp:
            variations.append({
                "text": conf_resp,
                "source": "confidence_computation",
                "score": 0.50,
                "style": "confidence",
            })

        # Strategy 105: Stream creation
        stream_resp = self._create_stream_response(entity, query)
        if stream_resp:
            variations.append({
                "text": stream_resp,
                "source": "stream_creation",
                "score": 0.50,
                "style": "streaming",
            })

        # Strategy 106: Location extraction
        loc_resp = self._extract_location(entity, query)
        if loc_resp:
            variations.append({
                "text": loc_resp,
        ...
        # Strategy 107: Math computation
        math_resp = self._compute_math(entity, query)
        if math_resp:
            variations.append({
                "text": math_resp,
        ...
        # Strategy 108: Sentence extraction
        sent_resp = self._extract_sentence(entity, query, candidates)
        if sent_resp:
            variations.append({
                "text": sent_resp,
        ...
        # Strategy 109: Topic extraction
        topic_resp = self._extract_topic(entity, query)
        if topic_resp:
            variations.append({
                "text": topic_resp,
        ...
        # Strategy 110: Goal state update
        goalstate_resp = self._update_goal_state(entity, query)
        if goalstate_resp:
            variations.append({
                "text": goalstate_resp,
                "source": "goal_state",
        ...
        # Strategy 111: Observation tracking
        obs_resp = self._track_observation(entity, query)
        if obs_resp:
            variations.append({
                "text": obs_resp,
                "source": "observation_tracking",
                "score": 0.50,
                "style": "observation",
            })

        # Strategy 112: Favorites management
        fav_resp = self._manage_favorites(entity, query)
        if fav_resp:
            variations.append({
                "text": fav_resp,
                "source": "favorites_management",
                "score": 0.50,
                "style": "favorites",
            })

        # Strategy 113: Recent entities
        recent_ent_resp = self._get_recent_entities(entity)
        if recent_ent_resp:
            variations.append({
                "text": recent_ent_resp,
                "source": "recent_entities",
                "score": 0.50,
                "style": "recent_entities",
            })

        # Strategy 114: Recent topics
        recent_topic_resp = self._get_recent_topics(entity)
        if recent_topic_resp:
            variations.append({
                "text": recent_topic_resp,
                "source": "recent_topics",
                "score": 0.50,
                "style": "recent_topics",
            })

        # Strategy 115: Conversation summary
        summary_resp = self._get_conversation_summary(entity)
        if summary_resp:
            variations.append({
                "text": summary_resp,
        ...
        # Strategy 116: Last turn context
        last_ctx_resp = self._get_last_turn_context(entity)
        if last_ctx_resp:
            variations.append({
                "text": last_ctx_resp,
                "source": "last_turn_context",
                "score": 0.50,
                "style": "context",
            })

        # Strategy 117: Entity state
        ent_state_resp = self._get_entity_state(entity)
        if ent_state_resp:
            variations.append({
                "text": ent_state_resp,
                "source": "entity_state",
        ...
        # Strategy 118: Original value
        orig_val_resp = self._get_original_value(entity, query)
        if orig_val_resp:
            variations.append({
                "text": orig_val_resp,
                "source": "original_value",
                "score": 0.50,
                "style": "original_value",
            })

        # Strategy 119: Recent context clues
        ctx_clues_resp = self._get_recent_context_clues(entity)
        if ctx_clues_resp:
            variations.append({
                "text": ctx_clues_resp,
        ...
        # Strategy 120: Entity context
        ent_ctx_resp = self._get_entity_context(entity, query)
        if ent_ctx_resp:
            variations.append({
                "text": ent_ctx_resp,
                "source": "entity_context",
                "score": 0.50,
                "style": "entity_context",
            })

        # Strategy 121: Related QA pairs
        qa_resp = self._get_related_qa_pairs(entity, query)
        if qa_resp:
            variations.append({
                "text": qa_resp,
                "source": "related_qa",
        ...
        # Strategy 122: Dataset loading
        ds_resp = self._load_dataset_qa(entity, query)
        if ds_resp:
            variations.append({
                "text": ds_resp,
                "source": "dataset_qa",
        ...
        # Strategy 123: Wrap-around facts
        wrap_resp = self._find_wrap_around_facts(entity, query)
        if wrap_resp:
            variations.append({
                "text": wrap_resp,
                "source": "wrap_around",
        ...
        # Strategy 124: Contrasting facts
        contrast_resp = self._find_contrasting_facts(entity, query)
        if contrast_resp:
            variations.append({
                "text": contrast_resp,
                "source": "contrasting",
        ...
        # Strategy 125: Attribute lookup
        attr_lookup_resp = self._lookup_attribute(entity, query)
        if attr_lookup_resp:
            variations.append({
                "text": attr_lookup_resp,
                "source": "attribute_lookup",
                "score": 0.50,
                "style": "attribute",
            })

        # Strategy 126: Turn count
        turn_resp = self._get_turn_count(entity)
        if turn_resp:
            variations.append({
                "text": turn_resp,
                "source": "turn_count",
        ...
        # Strategy 127: Emotional state
        emotion_state_resp = self._get_emotional_state(entity)
        if emotion_state_resp:
            variations.append({
                "text": emotion_state_resp,
                "source": "emotional_state",
                "score": 0.50,
                "style": "emotion",
            })

        # Strategy 128: Greeting detection
        greeting_resp = self._handle_greeting(entity, query)
        if greeting_resp:
            variations.append({
                "text": greeting_resp,
                "source": "greeting",
        ...
        # Strategy 129: Ambiguous entity
        amb_resp = self._handle_ambiguous_entity(entity, query)
        if amb_resp:
            variations.append({
                "text": amb_resp,
                "source": "ambiguous_entity",
                "score": 0.50,
                "style": "ambiguous",
            })

        # Strategy 130: ASCII art handling
        ascii_resp = self._handle_ascii_art(entity, query)
        if ascii_resp:
            variations.append({
                "text": ascii_resp,
                "source": "ascii_art",
        ...
        # Strategy 131: Multi-attribute query
        multi_resp = self._handle_multi_attribute_query(entity, query)
        if multi_resp:
            variations.append({
                "text": multi_resp,
                "source": "multi_attribute",
                "score": 0.50,
                "style": "multi",
            })

        # Strategy 132: OF pattern
        of_resp = self._handle_of_pattern(entity, query)
        if of_resp:
            variations.append({
                "text": of_resp,
                "source": "of_pattern",
        ...
        # Strategy 133: Natural talk
        natural_resp = self._handle_natural_talk(entity, query)
        if natural_resp:
            variations.append({
                "text": natural_resp,
                "source": "natural_talk",
        ...
        # Strategy 134: Context query
        ctx_query_resp = self._handle_context_query(entity, query)
        if ctx_query_resp:
            variations.append({
                "text": ctx_query_resp,
        ...
        # Strategy 135: Fact correction
        correction_resp = self._handle_fact_correction(entity, query)
        if correction_resp:
            variations.append({
                "text": correction_resp,
                "source": "fact_correction",
                "score": 0.50,
                "style": "correction",
            })

        # Strategy 136: Task command
        task_resp = self._handle_task_command(entity, query)
        if task_resp:
            variations.append({
                "text": task_resp,
                "source": "task_command",
        ...
        # Strategy 137: Hypothetical question
        hypo_resp = self._handle_hypothetical(entity, query)
        if hypo_resp:
            variations.append({
                "text": hypo_resp,
                "source": "hypothetical",
        ...
        # Strategy 138: Conversation history
        hist_query_resp = self._handle_conversation_history(entity, query)
        if hist_query_resp:
            variations.append({
                "text": hist_query_resp,
                "source": "conversation_history",
                "score": 0.50,
                "style": "history",
            })

        # Strategy 139: Yes/No question
        yesno_resp = self._handle_yes_no_question(entity, query)
        if yesno_resp:
            variations.append({
                "text": yesno_resp,
                "source": "yes_no",
        ...
        # Strategy 140: Learn command
        learn_cmd_resp = self._handle_learn(entity, query)
        if learn_cmd_resp:
            variations.append({
                "text": learn_cmd_resp,
    ...
    # ===== Helper methods for 140 strategies =====
    def _property_based_response(self, entity, query, candidates):
        if not entity or not self.ke: return None
        ent = self.ke.entities.get(entity.lower(), {})
        props = ent.get("properties", {})
        if props:
            items = list(props.items())[:2]
            facts = [f"The {k.replace('_',' ')} is {v}." for k,v in items]
            return " ".join(facts)
        return None

    def _attribute_based_response(self, entity, query, candidates):
        if not entity or not self.ke: return None
        ent = self.ke.entities.get(entity.lower(), {})
        attrs = ent.get("attributes", {})
        true_attrs = [k.replace('has_','').replace('is_','').replace('_',' ') for k,v in attrs.items() if v is True][:3]
        if true_attrs:
            return f"A {entity} is characterized by: {', '.join(true_attrs)}."
        return None

    def _find_similar_entity(self, entity, query):
        if not entity or not self.ke: return None
        cat = self.ke.get_category(entity)
        if cat:
            return f"{entity.capitalize()} belongs to the {cat} category."
        return None

    def _category_based_response(self, entity, query):
        if not entity or not self.ke: return None
        cat = self.ke.get_category(entity)
        return f"{entity.capitalize()} is classified under: {cat}." if cat else None

    def _relationship_based_response(self, entity, query):
        return None

    def _example_based_response(self, entity, query, candidates):
        if candidates:
            return candidates[0].get('text', '')[:100] if candidates else None
        return None

    def _contrast_based_response(self, entity, query, candidates):
        if entity.lower() == 'cat':
            return "Unlike dogs, cats are solitary hunters."
        return None

    def _historical_context(self, entity, query): return None
    def _usage_based_response(self, entity, query): return None
    def _origin_based_response(self, entity, query): return None
    def _common_knowledge_response(self, entity, query): return None
    def _scientific_classification(self, entity, query): return None
    def _fun_fact_response(self, entity, query, candidates): return None
    def _warning_response(self, entity, query): return None
    def _benefit_based_response(self, entity, query): return None
    def _characteristic_summary(self, entity, query, candidates): return None
    def _quick_fact_response(self, entity, query, candidates): return None
    def _detailed_breakdown(self, entity, query, candidates): return None
    def _simple_summary(self, entity, query, candidates):
        if candidates:
            return candidates[0].get('text', '')[:80] if candidates else None
        return None
    def _related_concept(self, entity, query): return None
    def _interesting_fact(self, entity, query, candidates): return None
    def _practical_application(self, entity, query): return None
    def _cultural_significance(self, entity, query): return None
    def _technical_detail(self, entity, query, candidates): return None
    def _visual_description(self, entity, query): return None
    def _comparative_analysis(self, entity, query): return None
    def _statistics_response(self, entity, query, candidates): return None
    def _timeline_response(self, entity, query): return None
    def _pros_cons_response(self, entity, query): return None
    def _myth_reality(self, entity, query): return None
    def _expert_opinion(self, entity, query): return None
    def _common_misconceptions(self, entity, query): return None
    def _real_world_example(self, entity, query): return None
    def _key_takeaway(self, entity, query, candidates): return None
    def _summary_with_emphasis(self, entity, query, candidates): return None
    def _narrative_response(self, entity, query, candidates): return None
    def _concise_answer(self, entity, query, candidates): return None
    def _elaborate_response(self, entity, query, candidates): return None
    def _question_based_response(self, entity, query, candidates): return None
    def _definition_response(self, entity, query, candidates): return None
    def _cause_effect(self, entity, query): return None
    def _step_by_step(self, entity, query, candidates): return None
    def _analogy_response(self, entity, query): return None
    def _expert_tip(self, entity, query): return None
    def _common_use_case(self, entity, query): return None
    def _related_field(self, entity, query): return None
    def _etymology_response(self, entity, query): return None
    def _related_phenomenon(self, entity, query): return None
    def _key_principle(self, entity, query): return None
    def _fundamental_concept(self, entity, query): return None
    def _practical_tip(self, entity, query): return None
    def _common_mistake(self, entity, query): return None
    def _best_practice(self, entity, query): return None
    def _key_metric(self, entity, query, candidates): return None
    def _industry_standard(self, entity, query): return None
    def _historical_development(self, entity, query): return None
    def _modern_interpretation(self, entity, query): return None
    def _scientific_principle(self, entity, query): return None
    def _realworld_application(self, entity, query): return None
    def _conceptual_explanation(self, entity, query): return None
    def _practical_implication(self, entity, query): return None
    def _theoretical_basis(self, entity, query): return None
    def _empirical_evidence(self, entity, query): return None
    def _comparative_perspective(self, entity, query): return None
    def _historical_precedent(self, entity, query): return None
    def _current_trends(self, entity, query): return None
    def _future_outlook(self, entity, query): return None
    def _key_challenges(self, entity, query): return None
    def _success_factors(self, entity, query): return None
    def _risk_factors(self, entity, query): return None
    def _environmental_factors(self, entity, query): return None
    def _social_impact(self, entity, query): return None
    def _economic_factors(self, entity, query): return None
    def _political_factors(self, entity, query): return None
    def _cultural_factors(self, entity, query): return None
    def _technological_factors(self, entity, query): return None
    def _legal_considerations(self, entity, query): return None
    def _ethical_considerations(self, entity, query): return None
    def _global_perspective(self, entity, query): return None
    def _detect_learning_intent(self, entity, query): return None
    def _extract_entity_info(self, entity, query, candidates): return None
    def _get_relative_time_context(self, entity, query): return None
    def _compute_health_assessment(self, entity, query): return None
    def _create_goal_tracking(self, entity, query): return None
    def _update_progress(self, entity, query): return None
    def _add_continuation(self, entity, query, candidates): return None
    def _add_followup(self, entity, query): return None
    def _create_plan(self, entity, query): return None
    def _format_duration_response(self, entity, query): return None
    def _format_value_response(self, entity, query): return None
    def _add_detail(self, entity, query, candidates): return None
    def _add_examples(self, entity, query, candidates): return None
    def _compute_confidence(self, entity, query): return None
    def _create_stream_response(self, entity, query): return None
    def _extract_location(self, entity, query): return None
    def _compute_math(self, entity, query): return None
    def _extract_sentence(self, entity, query, candidates): return None
    def _extract_topic(self, entity, query): return None
    def _update_goal_state(self, entity, query): return None
    def _track_observation(self, entity, query): return None
    def _manage_favorites(self, entity, query): return None
    def _get_recent_entities(self, entity): return None
    def _get_recent_topics(self, entity): return None
    def _get_conversation_summary(self, entity): return None
    def _get_last_turn_context(self, entity): return None
    def _get_entity_state(self, entity): return None
    def _get_original_value(self, entity, query): return None
    def _get_recent_context_clues(self, entity): return None
    def _get_entity_context(self, entity, query): return None
    def _get_related_qa_pairs(self, entity, query): return None
    def _load_dataset_qa(self, entity, query): return None
    def _find_wrap_around_facts(self, entity, query): return None
    def _find_contrasting_facts(self, entity, query): return None
    def _lookup_attribute(self, entity, query): return None
    def _get_turn_count(self, entity): return None
    def _get_emotional_state(self, entity): return None
    def _handle_greeting(self, entity, query): return None
    def _handle_ambiguous_entity(self, entity, query): return None
    def _handle_ascii_art(self, entity, query): return None
    def _handle_multi_attribute_query(self, entity, query): return None
    def _handle_of_pattern(self, entity, query): return None
    def _handle_natural_talk(self, entity, query): return None
    def _handle_context_query(self, entity, query): return None
    def _handle_fact_correction(self, entity, query): return None
    def _handle_task_command(self, entity, query): return None
    def _handle_hypothetical(self, entity, query): return None
    def _handle_conversation_history(self, entity, query): return None
    def _handle_yes_no_question(self, entity, query): return None
    def _handle_learn(self, entity, query): return None
    def _compare_entity(self, entity, query, candidates): return None
    def _get_temporal_context(self, entity, query): return None
...
class PipelineIntegrator:
    ...
    def process(self, query, num_responses=3):
        ...
        print(f"[DEBUG] process: generated {len(base_results)} base_results", flush=True)
        for r in base_results:
            print(f"[DEBUG]   result: source={r.get('source', 'unknown')}, score={r.get('score', 0):.3f}", flush=True)
...
class ChatEngine:
    ...
    def generate_top_responses(self, query, num_responses=3):
        ...
        if results:
            ...
            for r in results:
                cleaned = dict(r)
                cleaned['text'] = clean_response_text(r.get('text', ''))
                if cleaned['text']:
                    cleaned_results.append(cleaned)
            results = cleaned_results if cleaned_results else results
        ...
        if qa_results:
            # Clean all retrieved answers
            cleaned_results = []
            for q, a, s in qa_results:
                cleaned_text = clean_response_text(a)
                if cleaned_text:
                    cleaned_results.append({"text": cleaned_text, "source": "retrieval", "score": s})
            if cleaned_results:
        ...
        while True:
            ...
            try:
                ...
                for i, resp in enumerate(responses[:3], 1):
                    ...
                    display_text = clean_response_text(resp['text'])
                    print(f"  {i}. {display_text}", flush=True)
                    print(f"     [score={score:.3f} | source={source} | consistency={consistency}{extra}]", flush=True)
                print(flush=True)

                final_score = responses[0].get("tested_score", responses[0].get("score", 0)) if responses else 0
                print(f"[DEBUG] chat: total_responses={len(responses)}, final_score={final_score:.3f}", flush=True)
